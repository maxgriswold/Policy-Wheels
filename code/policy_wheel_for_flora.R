...
df <- fread('total_opioid_policies.csv')
df <- melt(df, id.vars = "st_cd", variable.name = "policy", value.name = "implemented")
...
df[implemented == ".", implemented := NA]
df[, implemented := as.Date(implemented, format = "%m/%d/%Y")]
...
svg(filename = "policy_wheel_flora.svg",
    width = 20, height = 12)
...
# ll <- par("usr")
# 
# text(1, 1, "Do you want a title Flora?", cex=1)
